...
public class CampaignsHelper {
    ...
    public Integer performActionsOnCampaign(final RequestSpecification requestSpec, final ResponseSpecification responseSpec,
        ...
        return Utils
                .performServerPost(requestSpec, responseSpec, SMS_CAMPAIGNS_ACTION_URL, getJSONForCampaignAction(command), "resourceId");
    }
    ...
    public String getJSONForCampaignAction(String command) {
        final HashMap<String, Object> map = new HashMap<>();
        String dateString = ("close".equalsIgnoreCase(command)) ? "closureDate" : "activationDate";
        map.put(dateString, Utils.getLocalDateOfTenant().toString(DATE_FORMAT));
        map.put("locale", "en");
        map.put("dateFormat", DATE_FORMAT);
        String json = new Gson().toJson(map);
        System.out.println(json);
