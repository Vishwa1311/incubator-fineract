...
public class MeetingWritePlatformServiceJpaRepositoryImpl implements MeetingWritePlatformService {
    ...
    public void updateCollectionSheetAttendance(final JsonCommand command) {
        ...
        try {
			...
			if(meeting==null){
				newMeeting=Meeting.createNew(calendarInstance, meetingDate, isTransactionDateOnNonMeetingDate,
	            		isSkipRepaymentOnFirstMonth, numberOfDays);;
			}
			else{
				if(!forcedSubmitOfCollectionSheet){
					throw new CollectionSheetHasAlreadyBeenSubmittedException(
							new LocalDate(meetingDate));
				}
				newMeeting=meeting;
			}
